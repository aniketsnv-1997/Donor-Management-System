...
class Modes(Resource):
    ...
    def get(self):
        ...
        return {"message": "No modes of donation exists in the system! Please add a new mode of donation"}, 401
...
class SingleMode(Resource):
    ...
    def post(self):
        ...
        parser.add_argument('mode_name', type=str, required=True, help="This is a mandatory field!")

        data = parser.parse_args()

        if MM.find_by_name(data['mode_name']):
            return {"message": f"Mode with name {data['mode_name']} is already present in the system!"}, 401

        new_mode = MM(None, data['mode_name'], dt.now(), None)
        new_mode.save_to_database()

        new_mode_added = MM.find_by_name(data['mode_name'])
        return {"id": new_mode_added.id, "mode_name": new_mode_added.mode_name,
                "create_date": str(new_mode_added.create_date), "update_date": new_mode_added.update_date}
    ...
    def delete(self, _id):
        ...
        if mode:
            ...
            return {"message": f"{deleted_mode_name} mode of donation has been deleted successfully from the system!"}

        return {"message": "Mode not present in the system!"}, 201