...
from dms.models.users.TokenModel import TokenModel
...
def get_token_id_from_jti(jti):
    return TokenModel.query.filter_by(jti=jti).first().id
...
def get_user_tokens(user_identity):
    ...
    given user
    """
    return TokenModel.query.filter_by(user_identity=user_identity).all()
...
def revoke_token(token_id, user):
    ...
    """
    try:
        token = TokenModel.query.filter_by(id=token_id, user_identity=user).one()
        token.revoked = True
        db.session.commit()
    except NoResultFound:
