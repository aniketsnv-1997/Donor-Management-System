...
from models.donors.StatesModel import StateModel
...
class State(Resource):
    ...
    def get(self):
        ...
        for state in states:
            states_list.append(
                {"id": state.id, "name": state.state_name, "country_id": state.country_id, "create_date": str(state.create_date),
...
class SingleState(Resource):
    ...
    def get(self, name):
        ...
        return {"id": state.id, "name": state.state_name, "country_id": state.country_id, "create_date": str(state.create_date),
                "update_date": str(state.update_date)}, 200
    ...
    def post(self, name):
        ...
        if StateModel.find_by_name(name):
            return {"message": f"State with name {name} is already present in the system!"}, 401
    ...
    def delete(self, name):
        state = StateModel.find_by_name(name)
        if state is None:
            return {"message": f"State with name {name} is not present in the system!"}, 401
