...
class UsersModel(db.Model):
    ...
    role_id = db.Column(db.INT, db.ForeignKey('roles.id'), nullable=False, unique=False)
    project_id = db.Column(db.INT, db.ForeignKey('projects.id'), nullable=False, unique=False)
    rights_id = db.Column(db.INT, db.ForeignKey('rights.id'), nullable=False, unique=False)
    create_date = db.Column(db.DATE, nullable=False, unique=False)
    update_date = db.Column(db.DATE, nullable=True, unique=False)

    role = db.relationship('RolesModel')
    project = db.relationship('ProjectsModel')
    right = db.relationship('RightsModel')
