from flask import (
    ...
    jsonify,
    request,
)  # Tell Python that we want to use Flask, jsonify - to convert dictionary
...
app = Flask(
    __name__
)  # "__name__" tells Flask that this application is running in some unique namespace
api = Api(app)
types = [
    {
        "name": "project",
...
def create_type():
    request_data = (
        request.get_json()
    )  # request.get_json() is used to fetch and collect the json data sent from
    ...
    return jsonify(
        {"types": types}
    )  # We return the jsonify version of the new data that was received from browser
...
def read_type():
    return jsonify(
        {"types": types}
    )  # A jsonify accepts only dictionaries, we passed a dummy key as 'types' and
